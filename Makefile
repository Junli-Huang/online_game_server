...
$(LUA_CLIB_PATH)/cjson.so : lualib-src/cjson/lua_cjson.c lualib-src/cjson/strbuf.c lualib-src/cjson/fpconv.c
	$(CC) $(CFLAGS) $(SHARED) $^ -o $@ -I$(LUADIR)

$(LUA_CLIB_PATH)/protobuf.so : | $(LUA_CLIB_PATH)
	cd lualib-src/pbc && $(MAKE) lib && cd binding/lua53 && $(MAKE) $(PLAT) && cd ../../../.. && mv lualib-src/pbc/binding/lua53/protobuf.so $@
